...
use crate::virtio::base::virtq_avail::VirtqAvail;
use crate::virtio::base::virtq_desc::VirtqDesc;
use crate::virtio::base::virtq_used::{VirtqUsed, VirtqUsedElem};
use crate::virtio::base::VirtqRingIndex;
...
impl<'a> DescriptorChain<'a> {
    fn checked_new(
        ...
        let virtq_desc = VirtqDesc::new(mem.clone(), desc_table, queue_size);
        let desc = virtq_desc.get(index).unwrap();
...
impl Queue {
    ...
    pub fn pop<'a, 'b>(&'a mut self, mem: &'b GuestMemory) -> Option<DescriptorChain<'b>> {
        let virtq_avail = VirtqAvail::new(mem.clone(), self.avail_ring, self.actual_size());
        if virtq_avail.idx().unwrap() == self.next_avail.0 {
            return None;
        }
        ...
        // offsets.
        let desc_index = virtq_avail.get(self.next_avail.0).unwrap();
    ...
    pub fn add_used(&mut self, mem: &GuestMemory, desc_index: u16, len: u32) {
        ...
        let virtq_used = VirtqUsed::new(mem.clone(), self.used_ring, self.actual_size());
        virtq_used
            .set(
                self.next_used.0,
                VirtqUsedElem {
        ...
        virtq_used.set_idx(self.next_used.0 as u16).unwrap();
    }
